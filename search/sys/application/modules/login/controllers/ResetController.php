<?php
class Login_resetController extends Common_LoginController {
	//初期化メソッドの定義
	private $_db;
			...
			if($this->_request->isPost()){
				...
				if($postArr['pw'] == $postArr['pw2']){
					...
					$user = $this->_db->fetchAll(
						//ユーザー情報を取得
						$this->_db->select()
							->from("passtoken",array('UserId','parent'))
							->where('token = ?',$token)
					);
					...
					if($this->_db->update("ImageUser",array("pw"=>$this->_db->pwHash($postArr['pw'])),"`id`=".$user[0]['UserId'])){
						//アップデート処理が完了した場合
						if($user[0]['parent']>0){
							$this->view->Msg2 = array('message'=>"パスワードのリセットが完了しました。<a href=\"".BASEURL."/login/?m=".$user[0]['parent']."\">ログインフォーム</a>よりログインしてください。",'color'=>'success');
						}else{
							$this->view->Msg2 = array('message'=>"パスワードのリセットが完了しました。<a href=\"".BASEURL."/login/\">ログインフォーム</a>よりログインしてください。",'color'=>'success');
						}
						//該当するリクエストを削除
