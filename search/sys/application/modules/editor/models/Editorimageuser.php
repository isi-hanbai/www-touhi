<?php
	class Model_Editorimageuser extends Model_Editorgeneral {

		//ユーザーリストの取得
		public function getUsers($kind =NULL,$keyword=NULL,$classes=NULL,$p=0,$limit=1,$parent){
			//SQLコマンドを作成
		...
		public function getUsersClasses($keyword=NULL,$p=0,$limit=1,$parent){
			//SQLコマンドを作成
			$sql = "SELECT DISTINCT u.*
						FROM userClassification u";
			//検索クエリを作成
			$whereArr = array();
			//検索キーワードが指定された場合
			if(!empty($keyword)){
			...
			$sql.= " LIMIT {$start} , {$limit}";
			//DBから取得
			$sql2 = "SELECT COUNT(*) AS n FROM userClassification u WHERE ".$where;
			$result = $this->_db->fetchAll($sql);
			$result2 = $this->_db->fetchAll($sql2);
			return array($result,$result2[0]['n']);
		}
		public function getUsersClassesDetail($id,$parent){
			//SQLコマンドを作成
			$sql = "SELECT u.* FROM userClassification u";
			//検索クエリを作成
			$whereArr = array();
			$whereArr[] = $this->_db->quoteInto("id=?",$id);
