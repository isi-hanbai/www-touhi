<?php
define("IMG_DIR","img/");
class Editor_SettingsController extends Common_EditorController {
	//初期化メソッドの定義
	private $_db;
	private $_user;
	public function init(){
		...
		$this->_db = new Model_Editorsettings();
	}
	//会社設定
	public function companyAction() {
		$this->view->title = '<i class="fa fa-cog"></i>　設定';
		$this->view->bread = '<li>
														<a href="'.BASEURL.'/editor/"><i class="fa fa-tachometer"></i>　ダッシュボード</a> <span class="divider">/</span>
														'.$this->view->title.'
													</li>';
		$this->view->data = $this->settings("company",$this->view->title);
		//$this->view->discount = $this->settings("discount",$this->view->title);
		//$this->view->tax = $this->settings("tax",$this->view->title);

		if($_GET['mode'] == "tax"){
			$this->view->Msg = "消費税設定の更新を行いました。";
		}elseif($_GET['mode'] == "discount"){
			$this->view->Msg = "割引設定の更新を行いました。";
		}elseif($_GET['mode'] == "company"){
			$this->view->Msg = "会社設定の更新を行いました。";
		}
	...
	public function globalAction() {
		$this->view->title = '<i class="fa fa-desktop"></i>　SMTP設定';
		$this->view->bread = '<li>
														<a href="'.BASEURL.'/editor/"><i class="fa fa-tachometer"></i>　ダッシュボード</a> <span class="divider">/</span>
														'.$this->view->title.'
													</li>';
		$this->view->data = $this->settings("global",$this->view->title);
	...
	public function indexAction() {
		...
		$this->view->bread = '<li>
														<a href="'.BASEURL.'/editor/"><i class="fa fa-tachometer"></i>　ダッシュボード</a> <span class="divider">/</span>
														<a href="'.BASEURL.'/editor/settings"　class="btn disabled"><i class="fa fa-cog"></i>　各種設定</a> <span class="divider">/</span>
													</li>';
	...
	public function profAction() {
		$this->view->title = '<i class="fa fa-cog"></i>　登録情報';
		$this->view->bread = '<li>
														<a href="'.BASEURL.'/editor/"><i class="fa fa-tachometer"></i>　ダッシュボード</a> <span class="divider">/</span>
														<a href="'.BASEURL.'/editor/settings"　class="btn disabled"><i class="fa fa-cog"></i>　各種設定</a> <span class="divider">/</span>
														'.$this->view->title.'
		...
		$res = $this->_db->fetchAll(
			$this->_db->select()
			->from("ImageUser")
			->where("id=?",$this->_user->parent)
		);
		$this->view->detail = $res[0];
	}
	//決済方法設定
	public function paymentAction() {
		...
		$this->view->bread = '<li>
														<a href="'.BASEURL.'/editor/"><i class="fa fa-tachometer"></i>　ダッシュボード</a> <span class="divider">/</span>
														<a href="'.BASEURL.'/editor/settings"><i class="fa fa-cog"></i>　各種設定</a> <span class="divider">/</span>
														'.$this->view->title.'
													</li>';
	...
	public function mailAction() {
		...
		if($this->_request->isPost()){
			...
			if($this->data[0]["count(*)"] > 0){
				$where = array();
				$where[] = $this->_db->quoteInto("parent=?",$this->_user->parent);
	...
	*/
	public function uploadAction() {
		if(is_uploaded_file($path = $_FILES['userfile']['tmp_name'])){
			...
			if($img_extension == "gif"){
				...
				$filename = $this->getRequest()->getPost('id')."stamp_".date("YmdHis").".gif";
			}elseif($img_extension == "jpg"){
				$exif_data = exif_read_data($path);
				$in = imagecreatefromjpeg($path); // 元画像ファイル読み込み
				$filename = $this->getRequest()->getPost('id')."stamp_".date("YmdHis").".jpg";
			}elseif($img_extension == "png"){
				$in = imagecreatefrompng($path); // 元画像ファイル読み込み
				$filename = $this->getRequest()->getPost('id')."stamp_".date("YmdHis").".png";
			}else{
	...
	private function settings($table,$ja){
		...
		if($this->_request->isPost()){
			...
			if($table == "company"){
				//非稼働時間の設定
				if(isset($postArr['restStart'])){
					$start = json_decode($postArr['restStart'],true);
					$startMinits = json_decode($postArr['restStartMinits'],true);
					$end = json_decode($postArr['restEnd'],true);
					$endMinits = json_decode($postArr['restEndMinits'],true);
					$rest = array();
					for($i=0;$i<count($start);$i++){
						$rest[] = array(
							"start"=>$start[$i],
							"startMinits"=>$startMinits[$i],
							"end"=>$end[$i],
							"endMinits"=>$endMinits[$i],
						);
					}
					$postArr['rest'] = json_encode($rest);
					//不要データの削除
					unset($postArr['restStart']);
					unset($postArr['restStartMinits']);
					unset($postArr['restEnd']);
					unset($postArr['restEndMinits']);
				}
				//休日の設定
				if(isset($postArr['holydayYear'])){
					$holydayYear = json_decode($postArr['holydayYear'],true);
					$holydayMonth = json_decode($postArr['holydayMonth'],true);
					$holydayDay = json_decode($postArr['holydayDay'],true);
					$holyday = array();
					for($i=0;$i<count($holydayYear);$i++){
						$holyday[] = array(
							"holydayYear"=>$holydayYear[$i],
							"holydayMonth"=>$holydayMonth[$i],
							"holydayDay"=>$holydayDay[$i]
						);
					}
					$postArr['otherHolyday'] = json_encode($holyday);
					//不要データの削除
					unset($postArr['holydayYear']);
					unset($postArr['holydayMonth']);
					unset($postArr['holydayDay']);
				}
			}
			if($this->_db->insertUpdate($table,$postArr,$this->_user->parent,$this->data[0]["count(*)"])){
				$this->view->Msg = $ja."の更新を行いました。";
			}
			if($table == "company"){
				header("location:".BASEURL."/editor/settings/company/?mode=".$table);
			}
		}
		$this->data = $this->_db->fetchAll(
			$this->_db->select()
