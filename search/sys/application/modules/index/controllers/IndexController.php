<?php
//コンポーネントのロード
class IndexController extends Common_IndexController {
	public $_setting;
	public $_db;
	private $_orign;
	//初期化メソッドの定義
	public function init(){
		...
		$this->origin = array(
			...
			"consultation"=>array("origin"=>"障害者福祉相談支援","sel"=>3),
			...
			"anesthesia"=>array("origin"=>"麻薬による疼痛管理","sel"=>1),
			...
			"swallowing"=>array("origin"=>"摂食嚥下訓練","sel"=>1),
			...
			"grindingIronAgent"=>array("origin"=>"錠剤の粉砕","sel"=>1),
		...
		*/
		...
		$sql = "SELECT
							...
							(
								...
								LEFT JOIN pref AS p ON p.id = i.pref
								LEFT JOIN facilityInfomation AS fi ON i.id = fi.user".$where."
							) AS c
					FROM ImageUser i
		...
		//var_dump($sql);
		//SELECT DISTINCT i.*, fi.bed, u.name AS className, p.name AS prefName, ( SELECT COUNT(*) FROM ImageUser as i LEFT JOIN facilityInfomation AS fi ON i.id = fi.user LEFT JOIN userClassification AS u ON u.id = i.class LEFT JOIN pref AS p ON p.id = i.pref WHERE i.kind=3 ) AS c FROM ImageUser i LEFT JOIN facilityInfomation AS fi ON i.id = fi.user LEFT JOIN userClassification AS u ON u.id = i.class LEFT JOIN pref AS p ON p.id = i.pref WHERE i.kind=3 ORDER BY `id` DESC
		if($result[0]['c'] < $limit){
			$this->end = $result[0]['c'];
		}
		$arr = array();
		...
		$this->view->pager = $this->userpager($keyword,$class,$procedure,$service,$area,$result[0]['c'],$limit,$p,"/index/index/list/");
		//echo COUNT($arr);
		/**/
	}
	public function detailAction() {
		...
		foreach($added[0] as $k=>$v){
			foreach($this->origin as $kkk=>$vvv){
				if($k == "added_".$kkk){
					if($vvv['sel'] ==1){
						$arr2["added_".$kkk] = array("v"=>$v,"sel"=>1,"label"=>$arr3[$kkk]);
						break;
					}else{
						$arr2["added_".$kkk] = array("v"=>$v,"sel"=>0,"label"=>$arr3[$kkk]);
						break;
					}
				}
			}
	...
	*/
	private function userpager($keyword="",$class,$procedure,$service,$area,$n=0,$limit=10,$p=0,$page){
		...
		if($n > $limit){
			...
			if(!empty($p)){
				$prev = $p-1;
				$pager.= "<li class=\"page-item{$avtive}\"><a class=\"page-link\" href=\"".BASEURL.$page."?".$class."&".$area."&".$procedure."&keyword=".$keyword."&limit=".$limit."&p=".$prev."\" aria-label=\"Previous\"><span aria-hidden=\"true\">&laquo; 前</span></a></li>";
			}
			if($p > $max){
				$pager.= "<li class=\"page-item{$avtive}\" ><a class=\"page-link\" href=\"".BASEURL.$page."?".$class."&".$area."&".$procedure."&keyword=".$keyword."&limit=".$limit."&p=0\">最初</a></li>";
			}
			for($i=0;$i<ceil($n/$limit);$i++){
				...
				if($i>$p+$max || $i < $p-$max){
					...
					$pager.= "<li class=\"page-item{$avtive}\" ><a class=\"page-link\" href=\"".BASEURL.$page."?".$class."&".$area."&".$procedure."&keyword=".$keyword."&limit=".$limit."&p=".$i."\">".$pn."</a></li>";
				}
			}
			if($p+$max < ceil($n/$limit)){
				$pager.= "<li class=\"page-item{$avtive} abc\" ><a class=\"page-link\" href=\"".BASEURL.$page."?".$class."&".$area."&".$procedure."&keyword=".$keyword."&limit=".$limit."&p=".(ceil($n/$limit)-1)."\">最後</a></li>";
			}
			if(ceil($n/$limit) > $p+1){
				$next = $p+1;
				$pager.= "<li class=\"page-item{$avtive}\"><a class=\"page-link\" href=\"".BASEURL.$page."?".$class."&".$area."&".$procedure."&keyword=".$keyword."&limit=".$limit."&p=".$next."\" aria-label=\"Next\"><span aria-hidden=\"true\">次&raquo;</span></a></li>";
			}
			$pager.= "</ul></nav></div>";
			return $pager;
		}
